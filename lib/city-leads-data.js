...
function getExtendedContentForCity(cityName) {
  return `${cityName}'s diverse economy supports a wide range of professional services and local businesses. From established firms to growing startups, the city offers rich opportunities for B2B lead generation and market research.\n\nUsing Geonayan, you can map this landscape by locality, compare density across areas, and export contact lists with phone numbers and emails for your sales or recruitment teams.`;
}

function getRelatedCitySlugs(currentSlug) {
  const all = getAllCitySlugs();
  const idx = all.indexOf(currentSlug?.toLowerCase());
  if (idx === -1) return all.slice(0, 3);
  const out = [];
  for (let i = 1; i <= 3; i++) out.push(all[(idx + i) % all.length]);
  return out;
}
...
export function getCityBySlug(slug) {
  ...
  return {
    ...
    extendedContent: getExtendedContentForCity(name),
    businessCategories: DEFAULT_BUSINESS_CATEGORIES,
    relatedSlugs: getRelatedCitySlugs(slugLower),
