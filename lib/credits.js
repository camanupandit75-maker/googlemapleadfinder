...
function cacheKey(query, location, provider, deep = false) {
  const normalized = `${(query || "").toLowerCase().trim()}|${(location || "").toLowerCase().trim()}|${provider}|${!!deep}`;
 ...
 * @param {boolean} [deep] - deep search cache variant
 * @returns {Promise<Array|null>} results or null
 */
export async function getCachedResults(query, location, provider, deep = false) {
  const key = cacheKey(query, location, provider, deep);
  const supabase = createServerSupabase();
  const { data, error } = await supabase
    .from("result_cache")
 ...
 * @param {boolean} [deep] - deep search cache variant
 */
export async function setCachedResults(query, location, provider, results, deep = false) {
  const key = cacheKey(query, location, provider, deep);
